...
* command expressions like `move rover 20 meters forward` that read like plain english
...
Now, this is just a simple function `move` with four arguments! _Done._
...
It would all be so much easier if we could modify the position...

=== Step 2: Introducing State

Luckily, there is a _State_ type in Frege and it perfectly fits our requirements.
...
* `State` has a `modify` operation that takes a high-order function, which knows how to
  update a position. Ha, this is exactly our new `move`!
...
Where there is "bind" (in the form of the `>>=` operator) there is also the do-nation that
we are using in the final solution. Here it is.
...
reads nicer. (There also is a `flip` function to do this in-place but I felt that to be distracting.)
