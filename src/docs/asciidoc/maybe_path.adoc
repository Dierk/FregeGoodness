...
In "<<fpath.adoc#fpath>>" we have experienced how to construct path expressions
...
Note the similarity to the domain in "<<fpath.adoc#fpath>>".
...
Looking at the involved functions and their types we see a similar pattern as in "<<fpath.adoc#fpath>>".
...
cases of "anchor:easy_io[Easy IO]" and "<<fpath.adoc#fpath>>", it is denoted with the `>>=` operator.
