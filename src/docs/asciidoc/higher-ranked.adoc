...
When such an annotation is present, instead of looking for the type a functional argument needs to be instantiated, the type checker just checks if the argument's type is _at least as general_ as the annotated type.
